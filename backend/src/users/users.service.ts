...
import { config } from '../config/config'; // 你的 config 模块/对象（JWT_SECRET 等）
...
export class UsersService {
  ...
  async validateUser(username: string, password: string): Promise<User | null> {
    ...
    if (!user) return null;
    if (!user.passwordHash) return null; // 无密码（可能为 OAuth 用户）
   ...
   */
  generateJwt(user: User): { accessToken: string; expiresIn: number } {
    const payload = { sub: user.id, username: user.username };
    const expiresIn = Number(process.env.JWT_EXPIRES_IN || config.JWT_EXPIRES_IN || 60 * 60 * 24); // seconds
   ...
   */
  private async initializeCardsForUser(user: User) {
